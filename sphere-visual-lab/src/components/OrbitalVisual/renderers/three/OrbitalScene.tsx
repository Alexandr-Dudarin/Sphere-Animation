...
export default function OrbitalScene({
  ...
  useFrame((state) => {
    ...
    if (rootRef.current) {
      ...
      rootRef.current.rotation.y = Math.cos(elapsed * 0.07 * safeSpeed) * 0.05;
      rootRef.current.rotation.z = Math.sin(elapsed * 0.05 * safeSpeed) * 0.016;
    }

    if (coreRef.current) {
      const breath = 1 + Math.sin(elapsed * 0.82 * safeSpeed) * 0.02;
      coreRef.current.scale.setScalar(breath);
      coreRef.current.rotation.z =
        Math.sin(elapsed * 0.14 * safeSpeed) * 0.04;
    }
  });
